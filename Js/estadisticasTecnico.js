...
function listo() 
{
    ...
    $('#pestaTab a').click(function(e) {
    	...
    	graficos(fechaYear);
    	graficos2(fechaYear);
    	$("#yearGraficoSelect option:first").attr("selected","selected");
	...
	{
    	var fec=$("#yearGraficoSelect3").val();
    	graficos2(fec);
	});
};
...
function graficos2(fecha)
{
	...
	arrayNumeros2[10]=0;
	arrayNumeros2[11]=0;
	var url = "http://127.0.0.1/Cantv/jsonCantv/consultarFallasSolucionadaUsuario.php?jsoncallback=?";
	$.getJSON(url,{id:ckie}).done(function(data){
		if(data.num != 0){
			var cantidad=0;
			$.each(data,function(i,item)
			{
