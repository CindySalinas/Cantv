...
$id= $_GET["idd"];
$sql = "SELECT F.Id_Enlace,E.Numero_Enlace,U.nombre_usuario,F.Id_Estatus,S.Estatus,F.Descripcion,F.Fecha,F.Hora,F.Id_Falla, U.Nombre, U.Apellido, C.Nombre, C.Id_Central, C.Latitud, C.Longitud, C.Direccion FROM fallas F INNER JOIN enlaces E ON F.Id_Enlace = E.Id_Enlace INNER JOIN estatus S ON F.Id_Estatus = S.Id_Estatus INNER JOIN usuarios U ON F.Id_Usuario_Falla = U.Id_Usuario INNER JOIN equipos Q ON E.Id_Equipo=Q.Id_Equipo INNER JOIN salas A ON Q.Id_Sala = A.Id_Sala INNER JOIN centrales C ON A.Id_Central=C.Id_Central WHERE F.Id_Falla='$id' ORDER BY F.Id_Falla DESC";
